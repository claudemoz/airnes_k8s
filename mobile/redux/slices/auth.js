...
import {
  ...
  verifyEmailService,
  reverifyEmailService,
  resetPasswordService,
  generateForgetPasswordlinkService,
  updateUserService,
    ...
    builder
      ...
      .addCase(login.fulfilled, (state, { payload: { token, profile } }) => {
        ...
        state.isAuthenticated =
          profile && Math.floor(Date.now() / 1000) < decodedToken?.exp;
      ...
      .addCase(fetchCurrentUser.fulfilled, (state, { payload }) => {
        ...
        state.isAuthenticated =
          payload?.profile && Math.floor(Date.now() / 1000) < decodedToken?.exp; // on vérifie que le user existe et que la date d'expiration du token est supérieur à la date du jour
      })
